...
import { formatDate, handleUpdateViews, slugify } from '@/app/lib/utils';
...
const RecentPublications = () => {
    ...
    useEffect(() => {
        const fetchPublications = async () => {
            try {
                const publicationsResponse = await api.get<Publication[]>(`/get-recent-publications`);
        ...
        <div className="container mx-auto px-2 py-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-4 gap-4">
                {publications.map((publication, index) => (
                    <div key={index} className="bg-white rounded-lg shadow-lg hover:shadow-xl  transition-transform duration-300 p-6 transform hover:scale-105">
                        ...
                        <Link onClick={(e) => handleUpdateViews(publication.id)} href={`/journals/${slugify(publication.category_name)}/${publication.id}`}>
